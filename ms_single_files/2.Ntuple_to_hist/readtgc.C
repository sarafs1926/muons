...
int linecount = 0 ;
std::string line ;
std::ifstream infile("tgcstations.txt") ;
if ( infile ) {
	while ( getline( infile , line ) ) {
		...
		for (Long64_t jentry=0; jentry < nentries; jentry++){ 
			...
			for (Long64_t kentry =0; kentry < hits_tgc_hitglobposx->size(); kentry++) { 
				...
				if (s1 == "tgc") {		
					if ( (hits_tgc_sim_stationname->at(kentry) == s2)  && (hits_tgc_sim_stationeta->at(kentry) == s3i) && (hits_tgc_sim_stationphi->at(kentry) == s4i) ) { 
						double x = hits_tgc_hitglobposx->at(kentry);
						//cout << "x= " << x << endl;
				   	 	double y = hits_tgc_hitglobposy->at(kentry);
						//cout << "y= " << y << endl;
						double z = hits_tgc_hitglobposz->at(kentry);
						//cout << "z= " << z << endl;
	...
	TFile *f = TFile::Open("tgchist0.root","UPDATE"); 
	f->cd();
