...
if ( infile ) {
	while ( getline( infile , line ) ) {
		...
		for (Long64_t jentry=0; jentry < nentries; jentry++){ 
			...
			for (Long64_t kentry =0; kentry < hits_rpc_hitglobposx->size(); kentry++) {
				...
				if (s1 == "rpc") {		
					if ( (hits_rpc_sim_stationname->at(kentry) == s2)  && (hits_rpc_sim_stationeta->at(kentry) == s3i) && (hits_rpc_sim_stationphi->at(kentry) == s4i) ) { 
	...
	TFile *f = TFile::Open("rpchist.root","UPDATE"); 
	f->cd();
