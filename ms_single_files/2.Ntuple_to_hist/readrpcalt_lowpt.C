...
if ( infile ) {
	while ( getline( infile , line ) ) {
		...
		cout << "s3i = " << s3i << endl;
		cout << "s4i = " << s4i << endl;
		std::string histname = s1 + s2 + s3o + s4o+ "_lowpt";
		...
		for (Long64_t jentry=0; jentry < nentries; jentry++){ 
			...
			for (Long64_t kentry =0; kentry < hits_rpc_hitglobposx->size(); kentry++) { 
				...
				if (s1 == "rpc" && s5 == "M") {		
					if ( (hits_rpc_sim_stationname->at(kentry) == s2)  && (hits_rpc_sim_stationeta->at(kentry) == s3i) && (hits_rpc_sim_stationphi->at(kentry) == s4i) && (hits_rpc_sim_doubletr->at(kentry) == 1) ) { 
	...
	TFile *f = TFile::Open("rpchist_lowpt.root","UPDATE"); 
	f->cd();
